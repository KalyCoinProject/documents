...
The below guide will instruct you on how to set up a kalychain network on your local machine for testing and development
purposes.

The procedure differs greatly from the way you would want to set up kalychain network for a real use scenario on
...
Refer to [Installation](/docs/get-started/installation) to install kalychain.

## Overview

![Local Setup](/img/ibft-setup/local.svg)

In this guide, our goal is to establish a working `kalychain` blockchain network working with [IBFT consensus protocol](https://github.com/ethereum/EIPs/issues/650).
...
`bootnode` is not a special instance of the kalychain node. Every kalychain node can serve as a `bootnode`, but
every kalychain node needs to have a set of bootnodes specified which will be contacted to provide information on how to connect with
...
* The `--ibft-validators-prefix-path` sets the prefix folder path to the one specified which IBFT in kalychain can
...
Because we are attempting to run a kalychain network consisting of 4 nodes all on the same machine, we need to take care to 
...
After running the previous commands, you have set up a 4 node kalychain network, capable of sealing blocks and recovering
from node failure.
...
A kalychain node can be started with a set **price limit** for incoming transactions.
...
Follow this guide to issue operator commands to the newly built cluster: [How to query operator information](/docs/working-with-node/query-operator-info) (the GRPC ports for the cluster we have built are `10000`/`38880`/`30000`/`40000` for each node respectively)