...
If you would like to setup a kalychain network locally to quickly test the `kalychain` before doing a production-like setup, please refer to
...
As the only part of the `kalychain` that needs to be exposed to other VMs is the libp2p server, simply allowing
...
Each of the 4 nodes will run on their own VM, as the idea of this guide is to give you a fully functional kalychain network while keeping the validator keys private to ensure a trustless network setup.
...
To get up and running with kalychain, you need to initialize the data folders, on each node:
...
`bootnode` is not a special instance of a kalychain node. Every kalychain node can serve as a `bootnode` and
