...
This guide goes into detail on how to back up and restore a kalychain node instance.
...
Kalychain leverages LevelDB as its storage engine.
When starting a kalychain node, the following sub-folders are created in the specified working directory:
...
Since the kalychain uses **LevelDB** for data storage, the node needs to be stopped for the duration of the backup, 
...
In order for the kalychain to use the restored data directory, at launch, the user needs to specify the path to the 
