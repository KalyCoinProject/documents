...
The modules of the kalychain **should not need to know how to keep secrets**. Ultimately, a module should not care if 
...
The node operator that's starting the kalychain can now specify which secrets manager they want to use, and as soon 
...
In order for the kalychain to be able to seamlessly communicate with the Vault server, it needs to parse an already
...
The kalychain uses the specified node name to keep track of the secrets it generates and uses on the Vault instance.
...
kalychain with the `server` command.
